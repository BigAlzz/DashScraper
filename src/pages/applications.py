...
dash.register_page(
    __name__,
    path='/applications',
    title='GCRA New Applications Dashboard',
    order=1
)
...
def process_image(contents):
    ...
    try:
        ...
        # Check if this is an applications dashboard
        if 'Renewals' in text:
            return None, "This appears to be a Renewals dashboard. Please use the Student Renewals Dashboard for this screenshot."
...
def load_last_statistics():
    ...
    try:
        ...
        # Always compare the last two entries
        if len(df) >= 2:
            current_stats = df.iloc[-1].to_dict()
            prev_stats = df.iloc[-2].to_dict()
            
            # Convert any NaN values to 0
            for key in current_stats:
                if pd.isna(current_stats[key]):
                    current_stats[key] = 0
                if pd.isna(prev_stats[key]):
                    prev_stats[key] = 0
                    
            return current_stats, prev_stats
        elif len(df) == 1:
            current_stats = df.iloc[-1].to_dict()
            # Convert any NaN values to 0
            for key in current_stats:
...
def generate_pdf_report(current_stats, prev_stats):
    ...
    try:
        ...
        pdf_filename = f"applications_report_{timestamp}.pdf"
        ...
        # Title and date
        story.append(Paragraph("GCRA Bursary Applications Dashboard", title_style))
        ...
        for section_title, metrics in sections.items():
            ...
            # Create table data
            table_data = [['Metric', 'Current', 'Previous', 'Change']]
            ...
            for label, key in metrics:
                ...
                table_data.append([label, f"{current:,}", f"{previous:,}", change])
                section_total_current += current
                section_total_prev += previous
            
            # Add section total with formatted numbers
            total_change = format_change(section_total_current, section_total_prev)[2]
            table_data.append(['Total', f"{section_total_current:,}", f"{section_total_prev:,}", total_change])
            
            # Create and style the table with adjusted widths for portrait
            ...
            trend_text = []
            for label, key in metrics:
                current, previous, _ = format_change(
                    current_stats.get(key, 0),
                    prev_stats.get(key, 0) if prev_stats else 0
                )
                if current != previous:
...
# Define the layout
layout = html.Div([
    # Add a dummy div for the download callback
    html.Div(id='dummy-output', style={'display': 'none'}),
    
    html.H1("GCRA New Applications Dashboard", style={'textAlign': 'center', 'marginBottom': '20px'}),
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div(id='upload-status-applications'),
            ])
        ], style={
            'flex': '2',
            'marginRight': '20px',
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(
                    id='previous-reports-applications',
            ...
            html.Div([
                ...
                html.Div(id='pdf-output-applications', style={'marginTop': '10px'})
            ])
        ], style={
            'flex': '1',
            'padding': '15px',
    ...
    html.Div([
        ...
        html.Pre(
            id='whatsapp-preview-applications',
            style={
                'whiteSpace': 'pre-wrap',
    ...
    # Statistics Display
    html.Div(id='statistics-display-applications', style={
        'display': 'grid',
    ...
    html.Div([
        ...
        dcc.Graph(id='trend-graph-applications')
    ], style={'margin': '20px', 'padding': '20px', 'backgroundColor': '#f8f9fa', 'borderRadius': '5px'}),
], style={'padding': '20px', 'fontFamily': 'Arial'})

# Add callbacks
@callback(
    [Output('upload-status-applications', 'children'),
     Output('statistics-display-applications', 'children'),
     Output('trend-graph-applications', 'figure'),
     Output('pdf-output-applications', 'children'),
     Output('whatsapp-preview-applications', 'children')],
    [Input('upload-image-applications', 'contents'),
     Input('generate-pdf-applications', 'n_clicks')],
...
def update_output(contents, n_clicks):
    ...
    if not triggered_id:
        try:
            ...
            stat_cards = create_stat_cards(current_stats, prev_stats)
            
            # Create trend graph
            fig = create_trend_graph()
            
            # Create WhatsApp preview
            whatsapp_text = create_whatsapp_preview(current_stats, prev_stats)
    ...
    if triggered_id == 'generate-pdf-applications' and n_clicks is not None:
        try:
            current_stats, prev_stats = load_last_statistics()
            if not current_stats:
    ...
    try:
        ...
        # Save the new statistics first
        save_statistics(stats)
        
        # Then load both current and previous stats for comparison
        current_stats, prev_stats = load_last_statistics()
...
def create_whatsapp_preview(current_stats, prev_stats):
    ...
    try:
        ...
        preview = [
            ...
            f"{report_type} Report - Applications",
...
# Add new callback for the reports dropdown
@callback(
    Output('previous-reports-applications', 'options'),
    Input('previous-reports-applications', 'id'),
    prevent_initial_call=False
)
def update_reports_dropdown(dummy):
    """Update the dropdown with available PDF reports from the last week."""
    try:
        reports_dir = Path.cwd() / 'reports'
        ...
        pdf_files = list(reports_dir.glob('applications_report_*.pdf'))
        
        # Filter and sort files from the last week
        ...
        for pdf_file in pdf_files:
            try:
                ...
                timestamp_str = pdf_file.stem.split('_')[-2] + '_' + pdf_file.stem.split('_')[-1]
                ...
                if file_date >= one_week_ago:
                    ...
                    recent_files.append({
                        'label': f"Applications Report - {date_display}",
                        'value': pdf_file.name
                    })
            except Exception as e:
        ...
        recent_files.sort(key=lambda x: x['value'], reverse=True)
        return recent_files
        
    except Exception as e:
        print(f"Error updating reports dropdown: {e}")
        return []

@callback(
    Output('dummy-output', 'children'),  # Dummy output to handle the download
    Input('previous-reports-applications', 'value'),
    prevent_initial_call=True
)
def download_selected_report(selected_report):
    """Handle the download when a report is selected."""
    if selected_report:
        return dcc.Location(pathname=f"/download/{selected_report}", id="download-redirect")
    return "" 